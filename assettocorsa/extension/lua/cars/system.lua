...
local function patchFn(g, name, n)
    ...
    if n == 2 then
        g[name] = function(a1, cb, ...)
            local r = appRunning
            return bak(a1, function(...)
                appRunning = r
                cb(...)
            end, ...)
        end
    ...
    if n == 3 then
        g[name] = function(a1, a2, cb, ...)
            local r = appRunning
            return bak(a1, a2, function(...)
                appRunning = r
                cb(...)
            end, ...)
        end
...
local function drawTopBar(space, transition)
    ...
    ui.childWindow('__topBar', vec2(space.x, system.topBarHeight), function()
        ui.drawRectFilledMultiColor(vec2(0, 0), vec2(space.x, system.topBarHeight), topBarColor, topBarColor,
        ...
        ui.drawTriangleFilled(c + vec2(52, 22), c + vec2(52 + 18 * s, 22), c + vec2(52 + 18 * s, 22 - 18 * s),
            rgbm.colors.white)
    end)
end
...
local function drawBottomBar(space, dt)
    ...
    if appStatus == false then
        appStatus = table.maxEntry(apps, function(app)
            return
                app.status ~= nil and app.statusPriority > 0 and (app == appCurrent and 0.001 or app.statusPriority) or
                    0
        end)
        if appStatus and (appStatus.statusPriority <= 0 or not appStatus.status) then
            appStatus = nil
        end
    ...
    -- Home button
    ui.drawCircleFilled(vec2(40, space.y + system.bottomBarHeight * 0.5),
        system.bottomBarHeight * (0.27 + 0.05 * btnHomePressed), rgbm.colors.white, 24)
    ui.drawCircle(vec2(40, space.y + system.bottomBarHeight * 0.5), system.bottomBarHeight * 0.35, rgbm.colors.white,
        24, 2)
    ui.setCursor(vec2(0, space.y))
    ui.invisibleButton('__btnHome', vec2(80, system.bottomBarHeight))
    if touchscreen.itemTapped() then
        if showPreviousApps then
    ...
    btnHomePressed = math.applyLag(btnHomePressed, ui.itemHovered() and ui.mouseDown(ui.MouseButton.Left) and 1 or 0,
        0.8, dt)
    ...
    if appStatus and appStatus.status then
        ...
        ui.drawRectFilled(vec2(80 + system.statusHeight / 2, space.y + m),
            vec2(80 + width - system.statusHeight / 2, space.y + system.statusHeight + m), system.statusColor)
        ui.drawEllipseFilled(vec2(80 + width - system.statusHeight / 2, space.y + system.bottomBarHeight / 2),
            system.statusHeight / 2, system.statusColor, 20)
        ui.drawEllipseFilled(vec2(80 + system.statusHeight / 2, space.y + system.bottomBarHeight / 2),
            system.statusHeight / 2, system.statusIconColor, 20)
        ui.offsetCursorY(m)
        local appStatusWide = true
        ui.childWindow('__status', vec2(width, system.statusHeight), function()
            ui.pushFont(ui.Font.Title)
            appRunning = appStatus
            local r = appStatus.status(dt)
            if r == false then
                appStatusWide = false
            elseif r == 'openonclick' and touchscreen.tapped() and ui.windowHovered() then
                system.openApp()
            end
            ui.popFont()
        end)
    ...
    if touchscreen.iconButton(newNotifications and ui.Icons.NotificationsAny or ui.Icons.Notifications,
        system.bottomBarHeight) then
        showNotifications = not showNotifications
        newNotifications = false
        if popupNotification then
...
local function updateCurrentAppCanvas(app)
    if app ~= nil then
        if app.canvas == nil then
            app.canvas = ui.ExtraCanvas(previewBaseSize)
        end
...
local function onAppOpen(app, args)
    ...
    if app.dynamicTextures then
        table.forEach(app.dynamicTextures, function(d)
            ac.setRenderingCameraActive(d, true)
        end)
    end
end
...
local function onAppClose(app)
    if app.displays then
        table.forEach(app.displays, function(d)
            ac.setDynamicTextureShift(d, 0)
        end)
    end
    if app.dynamicTextures then
        table.forEach(app.dynamicTextures, function(d)
            ac.setRenderingCameraActive(d, false)
        end)
    end
end
        ...
        if p.size then
            ...
            ui.childWindow(p.id, space, function()
                ...
                if p.inputValue ~= nil then
                    ...
                    ui.dwriteTextAligned(p.title, 20, ui.Alignment.Start, ui.Alignment.Start, vec2(space.x - 130, 36),
...
function system.addApp(appID, appFolder)
    ...
    if not io.fileExists(manifestFilename) then
        return
    end
    if io.fileExists(appFolderPath .. '/condition.lua') and require(appFolder .. '/condition') == false then
        return
    end
    ...
    -- App regristration for dashboard, matching dashboard/manifest.ini
    local app = {
        id = appID,
...
function system.openApp(app, moveBack, args)
    ...
    if type(app) == 'string' then
        app = table.findFirst(apps, function(a)
            return a.id == app
        end) or ac.warn('App is missing: ' .. app)
        if not app then
            return
        end
    end
...
function system.statusIcon(image, iconBgColor)
    if image == nil or iconBgColor == nil then
        image, iconBgColor = iconBgColor, image
    end
...
function system.setNotification(icon, content, details, silent, nonPersistent, launchArgs)
    ...
    if not data then
        ...
        table.sort(notificationsSorted, function(a, b)
            return a.app.notificationPriority == b.app.notificationPriority and a.app.name > b.app.name or
                       a.app.notificationPriority > b.app.notificationPriority
        end)
    end
...
function system.setPopupData(popupID, data)
    local p = table.findFirst(popups, function(p)
        return p.id == popupID
    end)
    if p then
        p._extraData = data
...
function system.getPopupData(popupID)
    local p = not popupID and popupRunning or table.findFirst(popups, function(p)
        return p.id == popupID
    end)
    if p then
        return p._extraData
...
function system.scrollList(dt, size, callback)
    if type(size) == 'function' then
        size, callback = ui.availableSpace(), size
    end
...
function system.scrollFadeTop()
    ui.drawRectFilledMultiColor(vec2(0, ui.getScrollY()), vec2(ui.windowWidth(), ui.getScrollY() + 8), system.bgColor,
        system.bgColor, rgbm.new(system.bgColor.rgb, 0), rgbm.new(system.bgColor.rgb, 0))
end
...
function system.contactIcon(carIndex, driverName, size)
    ...
    ui.dwriteTextAligned(firstLetter and firstLetter:upper() or '?', 30 * size / 40, ui.Alignment.Center,
        ui.Alignment.Center, vec2(size + 1, size - 2))
end
...
function system.setWallpaper(image)
    ...
    ui.ExtraCanvas(ui.imageSize(image)):update(function()
        ui.drawImage(image, 0, ui.windowSize())
    end):save(customFilename)
...
local function checkCompatibility()
    if size.y < 320 or size.x < 640 then
        return string.format(
            'Display is too small: %d×%d\nFor system to function, resolution should be at least 640×320', size.y,
            size.x)
    end
    if size.y > 800 or size.x > 1280 then
        return string.format(
            'Display is too large: %d×%d\nFor system to function, resolution should not be larger than 1280×800',
            size.y, size.x)
    end
    if size.x < size.y * 4 / 3 then
        return string.format(
            'Display is too narrow: %d×%d (%.2f)\nFor system to function, aspect ratio should be at least 4:3 (1.33)',
            size.y, size.x, size.x / size.y)
    end
    if car.year < 2015 and not ac.configValues({
        aftermarketComponent = false
    }).aftermarketComponent then
        return string.format(
            'Car is too old to have this system as a factory-installed component (this OS was released in 2015, car was made in %d)\nIf you are adding it as an aftermarket component, add “aftermarketComponent=1” to the config',
            car.year)
    end
end
...
local function allowScreenshots()
    if allowScreenshotsCache == nil then
        allowScreenshotsCache = ac.INIConfig.cspModule(ac.CSPModuleID.SmallTweaks):get('MISCELLANEOUS',
            'ANDROID_AUTO_TAKE_SCREENSHOT', false) or false
    end
...
local function actualUpdate(dt)
    ...
    if #popups > 0 and drawPopups(space, popupStoppedInput, dt) then
        popupStoppedInput = nil
    end
    if ac.isSkippingFrame() then
        return
    end
    ...
    if car.gas > 0.9 and car.brake > 0.9 and car.handbrake > 0.9 and screenshotTime <= 0 and allowScreenshots() then
        ...
        system.lastScreenshot:update(function()
            ui.drawImage('dynamic::android_auto', 0, ui.availableSpace(),
                ac.currentDisplayPosition() / ac.currentDisplayTextureResolution(), (ac.currentDisplayPosition() +
                    ac.currentDisplaySize()) / ac.currentDisplayTextureResolution())
        end)
    ...
    if screenshotTime > 0 then
        ...
        ui.childWindow('screenshot', ui.windowSize(), function()
            ...
            ui.drawRectFilled(ui.windowSize() * pad + offset - 8, ui.windowSize() * (1 - pad) + offset + 8,
                system.bgColor, 28)
            ui.drawImageRounded(system.lastScreenshot, ui.windowSize() * pad + offset,
                ui.windowSize() * (1 - pad) + offset, 20)
...
if ac.configValues({
    ...
    function system.update(dt)
        ...
        for i = 1, 9 do
            ui.drawLine(vec2(0, ui.windowHeight() * i / 10), vec2(ui.windowWidth(), ui.windowHeight() * i / 10),
                rgbm.colors.white, 2)
        end
        for i = 1, 19 do
            ui.drawLine(vec2(ui.windowWidth() * i / 20, 0), vec2(ui.windowWidth() * i / 20, ui.windowHeight()),
                rgbm.colors.white, 2)
        end
    end
    return
...
function system.update(dt)
    ...
    elseif #appsToAdd > 0 then
        ...
        for _, v in ipairs(ac.configValues({
            ExclusiveApp = {}
        }).ExclusiveApp) do
            io.loadAsync('%s/%s/extension/%s.zip' % {ac.getFolder(ac.FolderID.ContentCars), car:id(), v},
                function(err, data)
