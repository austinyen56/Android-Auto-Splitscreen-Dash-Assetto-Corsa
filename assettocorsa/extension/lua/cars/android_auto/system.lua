    ...
    if p.size then
      ...
      ui.childWindow(p.id, space, function ()
        ...
        if p.inputValue ~= nil then
          ...
          ui.popFont()
          appRunning, popupRunning = p.app, p
          p.callback(dt, p.inputValue, changed)
        else
...
function system.addApp(appID, appFolder)
  ...
  local app = {
    ...
    -- changed here
    widget = io.fileExists(__dirname..'/'..appFolder..'/widget.lua') and appFolder..'/widget' or nil,
...
local function actualUpdate(dt)
  ...
  if car.gas > 0.8 and car.brake > 0.8 and screenshotTime <= 0 then
