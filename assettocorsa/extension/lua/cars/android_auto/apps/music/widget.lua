...
local background = touchscreen.blurredBackgroundImage(rgbm(0.18, 0.18, 0.18, 1))
...
return function(dt, size)
    ...
    if easterEggNotificationTimer > 0 then
        easterEggNotificationTimer = easterEggNotificationTimer - dt
    ...
    local scale = math.min(size.x / referenceWidth, size.y / referenceHeight)
    ...
    local average = (accent.r + accent.g + accent.b) / 3
    ...
    ui.drawRectFilled(vec2(0, 0), size, rgbm(tintR, tintG, tintB, 0.18))

    -- Contrast layer: darkens shadows without strongly changing the cover’s overall color.
    ui.drawRectFilledMultiColor(vec2(0, 0), size, rgbm(0, 0, 0, 0.04), rgbm(0, 0, 0, 0.04), rgbm(0, 0, 0, 0.30),
        rgbm(0, 0, 0, 0.30))

    -- Light overlay at the top, darker near the bottom.
    ui.drawRectFilledMultiColor(vec2(0, 0), size, rgbm(1, 1, 1, 0.05), rgbm(1, 1, 1, 0.05), rgbm(0, 0, 0, 0.55),
        rgbm(0, 0, 0, 0.55))

    -- Additional subtle overall tint.
    ui.drawRectFilled(vec2(0, 0), size, rgbm(0, 0, 0, 0.08))
    ...
    ui.setCursor(vec2(topPadding, topPadding))

    ui.drawCircleFilled(vec2(topPadding + iconSize / 2, topPadding + iconSize / 2), iconSize / 2,
        rgbm(accent.r, accent.g, accent.b, 0.95), 24)

    ui.setCursor(vec2(topPadding, topPadding))

    ui.icon(ui.Icons.Music, vec2(iconSize, iconSize), rgbm.colors.white)
    ...
    local controlsY = size.y - buttonSize - controlsBottomPadding

    -- Responsive text stack

    local title = playing.title ~= '' and playing.title or 'Nothing Playing'

    -- Estimate whether the title needs two lines.
    local estimatedCharacterWidth = px(11)

    local estimatedTitleWidth = #title * estimatedCharacterWidth

    local wrapsTitle = estimatedTitleWidth > textWidth

    -- Dynamic title height.
    local titleHeight = wrapsTitle and px(62) or px(32)

    local titleArtistGap = px(6)

    local artistHeight = playing.artist ~= '' and px(24) or 0

    local artistProgressGap = px(12)

    local progressBarHeight = math.max(3, px(4))
    ...
    local textStackBottom = controlsY - stackBottomGap

    local titleY = textStackBottom - textStackHeight

    -- Title

    ui.setCursor(vec2(horizontalPadding, titleY))

    ui.dwriteTextAligned(title, px(23), ui.Alignment.Start, ui.Alignment.Start, vec2(textWidth, titleHeight), true,
        rgbm.colors.white)

    -- Artist

    local artistY = titleY + titleHeight + titleArtistGap

    if playing.artist ~= '' then
        ui.setCursor(vec2(horizontalPadding, artistY))

        ui.dwriteTextAligned(playing.artist, px(17), ui.Alignment.Start, ui.Alignment.Start,
            vec2(textWidth, artistHeight), true, rgbm(1, 1, 1, 0.82))
    end
    ...
    local progressBarY = artistY + artistHeight + artistProgressGap
    ...
    -- Unfilled bar.
    ui.drawRectFilled(vec2(progressBarX, progressBarY),
        vec2(progressBarX + progressBarWidth, progressBarY + progressBarHeight), rgbm(1, 1, 1, 0.32),
        progressBarHeight / 2)

    -- Filled bar.
    ui.drawRectFilled(vec2(progressBarX, progressBarY),
        vec2(progressBarX + progressBarWidth * progress, progressBarY + progressBarHeight), rgbm.colors.white,
        progressBarHeight / 2)
    ...
    if playing.trackPosition ~= -1 and playing.trackDuration ~= -1 then
        ...
        ui.dwriteTextAligned(formatTime(playing.trackPosition), px(12), ui.Alignment.Start, ui.Alignment.Start,
            vec2(progressBarWidth / 2, timeHeight), false, rgbm(1, 1, 1, 0.72))

        ui.setCursor(vec2(progressBarX + progressBarWidth / 2, timeY))

        ui.dwriteTextAligned(formatTime(playing.trackDuration), px(12), ui.Alignment.End, ui.Alignment.Start,
            vec2(progressBarWidth / 2, timeHeight), false, rgbm(1, 1, 1, 0.72))
    end
    ...
    local controlsWidth = buttonSize * 3 + preferredSpacing * 2

    -- Reduce spacing automatically on narrow panels.
    local maximumControlsWidth = size.x - horizontalPadding * 2

    local spacing = preferredSpacing

    if controlsWidth > maximumControlsWidth then
        spacing = math.max(px(8), (maximumControlsWidth - buttonSize * 3) / 2)

        controlsWidth = buttonSize * 3 + spacing * 2
    end

    local controlsX = (size.x - controlsWidth) / 2

    ui.setCursor(vec2(controlsX, controlsY))

    if touchscreen.iconButton(ui.Icons.Back, buttonSize) then
    ...
    local playButtonPosition = ui.getCursor()

    local playBorderPadding = px(9)

    ui.drawRectFilled(playButtonPosition - vec2(playBorderPadding, playBorderPadding),
        playButtonPosition + vec2(buttonSize, buttonSize) + vec2(playBorderPadding, playBorderPadding),
    ...
    if touchscreen.iconButton(playing.isPlaying and ui.Icons.Pause or ui.Icons.Play, buttonSize,
        ...
        if playClickCount >= 10 then
            system.setNotification(system.appIcon(), 'Secret Discovered: Play Button Spammer! ▶',
    ...
    if touchscreen.iconButton(ui.Icons.Next, buttonSize) then
        ac.mediaNextTrack()
    end
